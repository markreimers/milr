...
# function to plot avg signals and color NAs by probe type ----------------

signalAvgDiagnosticPlot = function(normedSignals, NAprobeRows=NULL,
                                   title=NULL){
  if(is.null(NAprobeRows)) 
    NAprobeRows = unique(which(is.na(normedSignals$log2NormedSignals[,,1]), arr.ind=T)[,1])
  if(is.null(title)) title = ' '
  plot(normedSignals$log2Standard, pch='.', col = rgb(0,0,1,alpha=.2),
       ,xlab = 'Avg Methylated', ylab = 'Avg Unmethylated', main=title)
  points(normedSignals$log2Standard[NAprobeRows,], pch = 16, 
         col = as.numeric(factor(normedSignals$probeSetInd[NAprobeRows])))
}


varExplPlot = function(normedSignals, title = 'Variance Explained'){
  library('vioplot', lib = '/home/manserpt/localR')
  
  plot(0,0, xlim = c(0,7), ylim=c(0,100), type='n',
       xlab='', ylab='', main=title)
  ii = 1
  probeLevels = c('II', 'Red', 'Grn')
  colInd = c('Red','Green', 'Red','Red','Green','Green')
  
  for(pp in 1:3){
    for(kk in 1:2){
      pSetTmp = which(probeSetInd==probeLevels[pp])
      varExplTmp = numeric(length(pSetTmp))
      for(jj in 1:length(pSetTmp)){
        vRaw = var(normedSignals$log2Deviatiosn[pSetTmp,jj,kk])
        vNorm = var(normedSignals$log2NormedSignals[pSetTmp,jj,kk]-normedSignals$log2standard[pSetTmp,kk])
        varExplTmp[jj] = 100*(vRaw-vNorm)/vRaw
      }
      vioplot(varExpl, at=ii, col=colInd[ii], add=T)
      ii=ii+1
    }
  }
